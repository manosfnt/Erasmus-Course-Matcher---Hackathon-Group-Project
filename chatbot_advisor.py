...
- If truly unrelated to Erasmus, decline in one sentence"""
    ...
    try:
        ...
        result = {"answer": response.choices[0].message.content or "Δεν μπορώ να απαντήσω με βασή το knowledge base μου."}
        if new_fact:
            result["new_fact"] = new_fact
        return result
    except Exception:
        return {"answer": "Δεν μπορώ να απαντήσω με βασή το knowledge base μου."}
